...
export default function Testimonials() {
  ...
  return (
    <section id="testimonials" className="section-padding bg-black-800">
        ...
        <div className="max-w-4xl mx-auto relative">
          <AnimatePresence mode="wait" custom={direction}>
            <motion.div key={current} custom={direction} variants={variants} initial="enter" animate="center" exit="exit"
              ...
              className="bg-black-700/50 backdrop-blur-sm rounded-3xl p-8 md:p-12 border border-black-600/30">
              ...
              <div className="flex items-center gap-4">
                <img src={t.image} alt={t.name} className="w-14 h-14 rounded-full object-cover border-2 border-orange-500" />
                <div>
                  <div className="text-white font-heading font-bold text-lg">{t.name}</div>
                  <div className="text-black-300 text-sm">{t.organization}</div>
                  <div className="flex gap-1 mt-1">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <Star key={i} className={`w-4 h-4 ${i < t.rating ? 'text-orange-500 fill-orange-500' : 'text-black-500'}`} />
                    ))}
                  </div>
                </div>
              </div>
          ...
          <div className="flex items-center justify-center gap-4 mt-8">
            <button onClick={prev} className="p-3 rounded-full bg-black-600/50 hover:bg-orange-500 text-white hover:text-black-900 transition-all duration-300">
              <ChevronLeft className="w-5 h-5" />
            </button>
            <div className="flex gap-2">
              {testimonials.map((_, i) => (
                <button key={i} onClick={() => { setDirection(i > current ? 1 : -1); setCurrent(i); }}
                  className={`transition-all duration-300 rounded-full ${i === current ? 'w-8 h-2 bg-orange-500' : 'w-2 h-2 bg-black-500 hover:bg-black-400'}`} />
              ))}
            </div>
            <button onClick={next} className="p-3 rounded-full bg-black-600/50 hover:bg-orange-500 text-white hover:text-black-900 transition-all duration-300">
