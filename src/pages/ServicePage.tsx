    ...
    <div className="min-h-screen">
      <div className="relative h-[50vh] min-h-[400px] flex items-center overflow-hidden">
        ...
        <div className="relative container-custom">
          <Link to="/" className="inline-flex items-center gap-2 text-white/80 hover:text-gold-400 transition-colors mb-6 text-sm">
        ...
        <div className="container-custom">
          <div className="grid lg:grid-cols-2 gap-12 items-start">
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
              <h2 className="font-heading text-3xl font-bold text-navy-800 mb-6">{cta}</h2>
              <p className="text-navy-600 leading-relaxed mb-8">{description}</p>
              <div className="space-y-3">
                {features.map((f) => (
                  <div key={f} className="flex items-center gap-3 p-3 rounded-xl bg-navy-50/50">
                    <CheckCircle className="w-5 h-5 text-gold-500 flex-shrink-0" />
                    <span className="text-navy-700 font-medium text-sm">{f}</span>
                  </div>
                ))}
              </div>
            </motion.div>
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
              <div className="bg-navy-50/50 rounded-2xl p-8">
                <h3 className="font-heading text-xl font-bold text-navy-800 mb-6">Our Process</h3>
                <div className="space-y-6">
                  {process.map((step, i) => (
                    <div key={step.title} className="flex gap-4">
                      <div className="w-10 h-10 rounded-xl bg-gold-500 flex items-center justify-center flex-shrink-0 font-heading font-bold text-navy-900 text-sm">
                        {String(i + 1).padStart(2, '0')}
                      </div>
                      <div>
                        <h4 className="font-heading font-bold text-navy-800 mb-1">{step.title}</h4>
      ...
      <section className="section-padding bg-navy-800 text-white text-center">
        <div className="container-custom max-w-2xl">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            ...
            <a href="tel:+919876543210" className="inline-flex items-center justify-center gap-2 border-2 border-white text-white font-semibold px-8 py-4 rounded-full hover:bg-white hover:text-navy-900 transition-all duration-300">
